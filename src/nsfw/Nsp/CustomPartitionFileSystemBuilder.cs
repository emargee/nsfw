...
/// <summary>
/// A modified version of <see cref="PartitionFileSystemBuilder"/> that allows for correct header padding scenarios.
/// </summary>
public class CustomPartitionFileSystemBuilder
{
    ...
    public CustomPartitionFileSystemBuilder() { }

    /// <summary>
    /// Creates a new <see cref="PartitionFileSystemBuilder"/> and populates it with all
    /// the files in the root directory.
    /// </summary>
    public CustomPartitionFileSystemBuilder(IFileSystem input)
    {
        using var file = new UniqueRef<IFile>();

        foreach (var entry in input.EnumerateEntries()
    ...
    private int CalcStringTableSize(int startOffset, PartitionFileSystemType type)
    {
        ...
        // Fix alignment - if the aligned size is exactly the same as the original size, we need to add 0x20 padding
        // This matches what N does when creating PFS0 files
    ...
    {
        public string Name = "";
        public IFile File = new NullFile();
        public long Length;
        public long Offset;

...
public class ScanMissingCommand : AsyncCommand<ScanMissingSettings>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, ScanMissingSettings settings)
    {
        ...
        foreach (var pair in missingList.OrderBy(x => x.Value.Name, StringComparer.InvariantCultureIgnoreCase))
        {
            ...
            {
                if (pair.Value.Name != null && (pair.Value.Name.ToLowerInvariant().EndsWith("demo") || pair.Value.Name.ToLowerInvariant().EndsWith("trial edition") || pair.Value.Name.ToLowerInvariant().StartsWith("demo:") || pair.Value.Name.ToLowerInvariant().EndsWith("(demo)") || pair.Value.Name.ToLowerInvariant().Contains("trial version")))
                {
